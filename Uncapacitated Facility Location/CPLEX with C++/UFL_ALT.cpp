...
// Define variable array and matrix of arc lengths
...
int main()
{
	...
	try
	{
		...
		// Constraint 3 (Equation 5 in the pdf formulation) - for each facility (If open), total quantity delivered to all clients should be lesser than or equal to net demand of all clients
		for (q = 0; q < clients; q++) {
			for (p = 0; p < fac; p++) {
				IloExpr constr5(env);
				constr5 = x[p][q];
				mod.add(constr5 <= client_dem[q] * y[p]);
				constr5.end();
			}
		...
		cout << endl << "(a) List of facilities with non-zero production in the optimal solution with the total production at each of these facilities" << endl;
		...
		// Part i
		// No. of cuts
		// cout << "no. of cuts = " << cplex.getNcuts(IloCplex::CutMir);
		...
		cout << "(h) Percentage of gap between LP optimal objective value and MIP objective value = " << 100 * (MIPobjVal - LPobjVal) / MIPobjVal << "%" << endl;
